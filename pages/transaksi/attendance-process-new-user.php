<?php
    ...
    $caritbuser = mysql_query("SELECT * FROM tb_user WHERE id_user='$id_user'");
    $ctb = mysql_fetch_assoc($caritbuser);
    $divisi = $ctb['divisi'];
    $div2   = $ctb['div2'];
    $myArray = explode(',', $div2);
    ...
    switch ($action) {
        ...
        case 'tampilkan_absen':
            ...
            $q_koreksiabsen = mysql_query("SELECT * FROM koreksiabsen WHERE tanggal BETWEEN '$awal' AND '$akhir'");
            $koreksiabsen = array();
            while ($row = mysql_fetch_array($q_koreksiabsen))
            {
              $koreksiabsen[$row['id_absen'].$row['tanggal']] = $row;
            }
            ...
            $absen .= "<th colspan='5' width='250px' bgcolor='#155050'>Akumulasi</th>";
            $absen .= "</tr>";
            $absen .= "<tr>";
                $d = $tgl;
                $t = intval($x->format('t'));
            ...
            $absen .= "<th rowspan='2' width='50px' bgcolor='#155050'>C</th>";
            $absen .= "<th rowspan='2' width='50px' bgcolor='#155050'>M</th>";
            $absen .= "<th rowspan='2' width='50px' bgcolor='#155050'>SDSD</th>";
            $absen .= "<th rowspan='2' width='50px' bgcolor='#155050'>STSD</th>";
            $absen .= "<th rowspan='2' width='50px' bgcolor='#155050'>UP</th>";
            $absen .= "</tr>";
            $absen .= "<tr>";
                $d = $tgl;
                $t = intval($x->format('t'));
            ...
            $absen .= "<tbody>";
                ...
                $user_query = mysql_query("SELECT
                                                ...
                                                mitra AS statusmitra
                                            FROM
                                                tb_user
                                            WHERE
                                                div2 IN ($result) OR div2 = '$div2';
                                                ");
                $no     = 1;
                while ($row = mysql_fetch_array($user_query)) {
                    ...
                    for ($i=1; $i <= $jml_hari; $i++) {
                        ...
                        if ($hari=='Sat' || $hari=='Sun') { // Sabtu / Minggu
                            ...
                            if ($kal == 1) {
                                ...
                                if ($izin1 == 1) { // Jika Form Izin ada
                                    ...
                                    $btn = "onclick='window.open(\"https://180.211.92.132/hconline/izin/".$izin_nonabsen[$row['inisial_id'].$ymd]['gambar']."\", \"newwindow\", \"width=810,height=900\")'";
                                    ...
                                    if ($ada_cuti == 1) { // Jika Form Cuti Ada
                                        ...
                                        $btn        = "onclick='window.open(\"https://180.211.92.132/hconline/uploads/".$cuti[$row['inisial_id'].$ymd]['gambar']."\", \"newwindow\", \"width=810,height=900\")'";
                                    }
                                        ...
                                        if ($ada_unpaid == 1) { // Jika Form Cuti Ada
                                            ...
                                            $btn        = "onclick='window.open(\"https://180.211.92.132/hconline/uploads/".$unpaid[$row['inisial_id'].$ymd]['gambar']."\", \"newwindow\", \"width=810,height=900\")'";
                                        }elseif ($ymd >= date('Y-m-d')) {
                                            $warna  = "bgcolor='#fffeef'";
                                        }
                                        else { // Jika tidak abasen & tidak ditemukan data izin dan cuti
                                          if($row['statusmitra'] == '1'){
                                          $warna  = "bgcolor=''";
                                          $simbol = "";
                                          }else{
                        ...
                        // $rangejam = $absen_array[$row['user_id'].$ymd]['jam_masuk'];
                        $key = $row['user_id']."-".$row['inisial_id']."-".$ymd;

                        $ka = isset($koreksiabsen[$row['user_id'].$ymd]) == true ? "<font color='magenta'>KA</font><br>" : "";

                        $absen .= "<td ".$warna." ".$btn."><center>".$telat.$ka.$simbol.$rangejam."</center></td>";
                        $d++;
                    }
