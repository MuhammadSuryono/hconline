...
<?php
  ...
  // $get = mysql_query("SELECT no FROM tb_izin ORDER BY no DESC LIMIT 1");
                            ...
                            <div class='box box-primary'>
                              <div class='register-box-body'>
                                ...
                                <div class='row'>
                                  <div class='col-xs-8'></div>
                                  <div class='col-xs-4'>
                                    <button type='button' onclick=location.href='home-pegawai.php?page=history-izin-pegawai' class='btn btn-block btn-warning'>Next</button>
                                  </div>
                                </div>
                              </div>
                            </div>";
  ...
  else
  {
    if ($hakcutifinal < $workingDays){
      ...
      // echo $workingDays;
        ...
        // $insert =mysql_query("INSERT INTO tb_izin (no,nip,nama,divisi,tgl,dari,sampai,jml_hari,jenis,gambar)
        //                       VALUES ('$no','$nip','$nama','$divisi','$tgl','$daritgl','$sampaitgl','$workingDays','$jenis','$nama_gambar')");
         $insert =mysql_query("SELECT hak_cuti_tahunan,hak_cuti_tahunlalu FROM tb_pegawai WHERE nip='$nip'");
  ...
  if ($jenis == "Sakit Tanpa Surat Dokter"){
        ...
        if($haktahunlalu >= $workingDays){
          $potongtahunlalu = mysql_query("UPDATE tb_pegawai SET hak_cuti_tahunlalu = (hak_cuti_tahunlalu - $workingDays) WHERE nip='$nip'");
          $jarak = TRUE;
        }
        else if($haktahunlalu < $workingDays){
          $tinggalhaknya = $hakcutifinal - $workingDays;
          $potongcutigabung = mysql_query("UPDATE tb_pegawai SET hak_cuti_tahunan='$tinggalhaknya' WHERE nip='$nip'");
          $potongyangtahunlalu = mysql_query("UPDATE tb_pegawai SET hak_cuti_tahunlalu = 0 WHERE nip='$nip'");
          $jarak = TRUE;
        }
          ...
          if($haktahunlalu >= $workingDays) {
            $tambah =mysql_query("INSERT INTO tb_izin (no,nip,nama,divisi,tgl,dari,sampai,jml_hari,jenis,gambar,cutisebelum,cutisesudah)
                               VALUES ('$no','$nip','$nama','$divisi','$tgl','$daritgl','$sampaitgl','$workingDays','$jenis','$nama_gambar', '$haktahunlalu', '$hak2_update')");
          } else {
            $tambah =mysql_query("INSERT INTO tb_izin (no,nip,nama,divisi,tgl,dari,sampai,jml_hari,jenis,gambar,cutisebelum,cutisesudah)
                               VALUES ('$no','$nip','$nama','$divisi','$tgl','$daritgl','$sampaitgl','$workingDays','$jenis','$nama_gambar', '$haktahunini', '$hak_update')");
          }
    }
  ...
  if ($jarak == TRUE)
  {
    ...
    foreach($daterange as $date){
      ...
      $insertrange = mysql_query("INSERT INTO daterange_izin (no_izin,username,tanggal,jenis,persetujuan,gambar,id_absen) VALUES ('$no','$nip','$tanggal','$jenis','','$nama_gambar')");
