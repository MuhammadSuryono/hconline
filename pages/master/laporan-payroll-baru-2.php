...
<div class="x_panel" style="">
    <div class="x_content">
        <div class="well" style="overflow: auto">
            ...
            ?>
            <form action="<?php echo $link; ?>?page=laporan-payroll-baru-2" method="post">
                <div class="col-md-4">
                    <div class="input-prepend input-group">
                        <span class="add-on input-group-addon"><i class="glyphicon glyphicon-calendar fa fa-calendar"></i></span>
                        <select class="form-control" name="range_default" onchange="dateRange(this.value)">
                            <option value='cutoff' <?php if ($range_default == 'cutoff') {
                                                        echo "selected";
                                                    } ?>><?php echo $text_default; ?></option>
                            <option value='custom' <?php if ($range_default == 'custom') {
                                                        echo "selected";
                                                    } ?>>Custom</option>
                        </select>
                    </div>
                ...
                <div class="col-md-4">
                    <div class="btn-group">
                        <button type="submit" class="btn btn-info"><i class="fa fa-eye"></i> Tampilkan</button>
                        <button type="button" onclick="collapse()" class="collapse-all btn btn-warning"><i class="fa fa-expand"></i> Collapse All</button>
                    </div>
                </div>
            </form>
        </div>
...
<script>
    ...
    function collapse() {
        var numPanelOpen = $('.collapse.in').length;
        if (numPanelOpen == 0) {
            $('.panel-collapse:not(".in")').collapse('show');
            $('.collapse-all').html('<i class="fa fa-compress"></i> Minimize All');
        } else {
            $('.panel-collapse.in').collapse('hide');
            $('.collapse-all').html('<i class="fa fa-expand"></i> Collapse All');
        }
    }
    ...
    $(document).ready(function() {
        ...
        $.ajax({
                url: "pages/transaksi/proses-laporan-payroll-baru-2.php",
                dataType: 'html',
                method: 'post',
                data: {
                    tgl_1: tgl_1,
